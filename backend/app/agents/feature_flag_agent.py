...
AGENT_CONTRACT: dict[str, Any] = {
    ...
    "side_effects": ["writes feature flag audit reports"],
    "permissions": ["read_repo", "write_docs"],
    "risk_level": "low",
    "expected_verification": {"read": "search_code must run to find all flag usages"},
    "dependencies": [],
}

_SUBMIT = {
    "name": "submit_feature_flag_agent",
    "description": "Submit feature_flag_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
_WRITE = {
    "name": "write_file",
    "description": "Write feature flag audit report.",
    "input_schema": {
        "type": "object",
...
def make_feature_flag_agent_handlers(repo_path: str) -> dict[str, Any]:
    ...
    base["submit_feature_flag_agent"] = submit_h
    base["_result"] = result
    return base


def run_feature_flag_agent(
    task_id: int,
    description: str,
    ...
    handlers = make_feature_flag_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Use search_code to find all feature flag definitions and usages in the codebase.\n"
        "2. Use find_references to check which flags are still actively referenced vs. dead code.\n"
        "3. For each stale flag: cite the flag name, file:line where it's defined, and the removal steps.\n"
        "4. Flag any flags permanently set to true/false (candidate for removal).\n"
        "5. Write an audit report with write_file if requested.\n"
        "6. Call submit_feature_flag_agent with summary, findings, and recommendations."
    )
