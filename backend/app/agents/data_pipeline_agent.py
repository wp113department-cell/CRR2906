"""data_pipeline_agent — designs and reviews data pipelines, ETL flows, and schemas."""
...
AGENT_CONTRACT: dict[str, Any] = {
    ...
    "description": "Designs, reviews, and documents data pipelines, ETL flows, and data schemas.",
    "allowed_tools": [
        "read_file",
        "list_files",
        "search_code",
    ...
    "side_effects": ["writes pipeline design .md or .yaml files"],
...
_SUBMIT = {
    "name": "submit_data_pipeline_agent",
    "description": "Submit data_pipeline_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
_WRITE = {
    "name": "write_file",
    "description": "Write pipeline design or documentation file.",
    "input_schema": {
        "type": "object",
...
def make_data_pipeline_agent_handlers(repo_path: str) -> dict[str, Any]:
...
def run_data_pipeline_agent(
    ...
    handlers = make_data_pipeline_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Use read_file and search_code to understand existing data flows and schemas.\n"
        "2. Identify data sources, transformations, sinks, and quality checks needed.\n"
        "3. Design the pipeline: ingestion → transformation → validation → loading steps.\n"
        "4. Document data contracts (input/output schemas, SLAs, error handling).\n"
        "5. Write the design doc with write_file if requested.\n"
        "6. Call submit_data_pipeline_agent with summary, findings, and recommendations."
    )
    ...
    try:
        ...
        register(
            AgentCapability(
                ...
                capabilities=["data_pipeline_design"],
