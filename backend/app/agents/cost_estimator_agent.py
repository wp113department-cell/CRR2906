...
AGENT_CONTRACT: dict[str, Any] = {
    ...
    "side_effects": ["writes cost estimate .md reports"],
...
_SUBMIT = {
    "name": "submit_cost_estimator_agent",
    "description": "Submit cost_estimator_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
_WRITE = {
    "name": "write_file",
    "description": "Write cost estimate report.",
    "input_schema": {
        "type": "object",
...
def make_cost_estimator_agent_handlers(repo_path: str) -> dict[str, Any]:
    ...
    base["submit_cost_estimator_agent"] = submit_h
    base["_result"] = result
    return base


def run_cost_estimator_agent(
    task_id: int,
    description: str,
    ...
    handlers = make_cost_estimator_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Use read_file, search_code, and analyze_file to understand current codebase scope.\n"
        "2. Identify which files/modules will be touched, their complexity, and risk.\n"
        "3. Produce: story point estimate, time estimate (hours), risk factors,\n"
        "   and token cost estimate for the agent pipeline that will execute this work.\n"
        "4. Write the estimate report with write_file if requested.\n"
        "5. Call submit_cost_estimator_agent with summary, findings, and recommendations."
    )
