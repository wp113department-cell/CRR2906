...
AGENT_CONTRACT: dict[str, Any] = {
    ...
    "side_effects": ["writes quality report .md files"],
...
_SUBMIT = {
    "name": "submit_code_quality_agent",
    "description": "Submit code_quality_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
_WRITE = {
    "name": "write_file",
    "description": "Write quality report file.",
    "input_schema": {
        "type": "object",
...
def make_code_quality_agent_handlers(repo_path: str) -> dict[str, Any]:
...
def run_code_quality_agent(
    ...
    handlers = make_code_quality_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Use read_file, parse_ast, analyze_file to inspect the target code.\n"
        "2. Check for: complexity, duplication, naming, missing types, TODO debt.\n"
        "3. Use find_todos and search_code to find quality issues across files.\n"
        "4. Write a quality report with write_file if requested.\n"
        "5. Call submit_code_quality_agent with summary, findings, recommendations."
    )
