"""runbook_generator_agent — writes operational runbooks from actual service and infra code."""
...
AGENT_CONTRACT: dict[str, Any] = {
    ...
    "description": "Writes operational runbooks from actual service code and deployment config. Each runbook step is a concrete command with an observable success signal, never invented from memory.",
    "allowed_tools": [
        "read_file",
        "list_files",
        "search_code",
    ...
    "expected_verification": {
        "read": "read_file must run to inspect service and config files before writing runbook"
    },
    "dependencies": [],
}

_SUBMIT = {
    "name": "submit_runbook_generator_agent",
    "description": "Submit runbook_generator_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
_WRITE = {
    "name": "write_file",
    "description": "Write operational runbook.",
    "input_schema": {
        "type": "object",
...
def make_runbook_generator_agent_handlers(repo_path: str) -> dict[str, Any]:
...
def run_runbook_generator_agent(
    task_id: int,
    description: str,
    ...
    handlers = make_runbook_generator_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Read the actual service code, Dockerfile, deployment config, and health check endpoints.\n"
        "2. State what the service does and how it's deployed before drafting anything.\n"
    ...
    try:
        ...
        register(
            AgentCapability(
                ...
                capabilities=["runbook_generation"],
