...
_SUBMIT = {
    "name": "submit_onboarding_agent",
    "description": "Submit onboarding_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
def make_onboarding_agent_handlers(repo_path: str) -> dict[str, Any]:
...
def run_onboarding_agent(
    ...
    handlers = make_onboarding_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Read README, Makefile, .env.example, docker-compose.yml, and CI configs.\n"
        "2. State what you found before drafting anything — what's missing, what's unclear.\n"
