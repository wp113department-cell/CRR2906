...
_SUBMIT = {
    "name": "submit_rollback_agent",
    "description": "Submit rollback_agent result.",
    "input_schema": {
        "type": "object",
        "properties": {
            "summary": {"type": "string"},
...
def make_rollback_agent_handlers(repo_path: str) -> dict[str, Any]:
    ...
    base["submit_rollback_agent"] = submit_h
    base["_result"] = result
    return base
...
def run_rollback_agent(
    ...
    handlers = make_rollback_agent_handlers(repo)
    result = handlers["_result"]

    msg = (
        f"Task #{task_id} — {description}\n\n"
        "1. Use git_log to identify the specific deployment or migration to roll back.\n"
        "2. Use git_diff and git_show to understand exactly what changed.\n"
        "3. Read the migration history to confirm if a DB rollback is required.\n"
        "4. Produce a step-by-step rollback plan — each step must have a verifiable outcome.\n"
        "5. Flag which steps require human sign-off before execution.\n"
        "6. Write the rollback plan with write_file.\n"
        "7. Call submit_rollback_agent with summary, findings, and recommendations."
    )

    final_state = run_agent_graph(
        role_name="rollback_agent",
