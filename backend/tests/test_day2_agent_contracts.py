...
_FINAL_STATE: dict[str, Any] = {
    ...
    "result": {
        ...
        "findings": [],
        "summary": "ok",
        "structure_summary": "1 module",
        "risks": [],
        "recommendations": [],
        "blast_radius": "low",
        "query_or_migration": "SELECT 1",
        "explain_plan_summary": "ok",
        "verified_against_schema": True,
        "is_destructive": False,
        "warnings": [],
        "files_written": [],
        "files_changed": [],  # noqa: F601
        "status": "healthy",
        "metrics": {},
        "issues": [],
        "dependencies": [],
        "content_markdown": "# README",
        "verified_commands": [],
        "sections": [],
        "endpoints": [],
        "spec_drift": [],
...
def _assert_all_flags(kwargs: dict[str, Any], agent_name: str) -> None:
    ...
    assert kwargs.get(
        "task_description"
    ), f"{agent_name}: task_description must be non-empty"
...
class TestAgentContractFormat:
    ...
    def test_contract_risk_level_valid(self, module_path: str) -> None:
        ...
        contract: dict[str, Any] = getattr(mod, "AGENT_CONTRACT")
        assert (
            contract["risk_level"] in _ALLOWED_RISK_LEVELS
    ...
    def test_contract_description_non_empty(self, module_path: str) -> None:
        ...
        assert contract[
            "description"
        ].strip(), f"{module_path}: AGENT_CONTRACT description must be non-empty"
...
class TestCapabilityRegistration:
    ...
    def test_all_day2_agents_registered(self) -> None:
        ...
        for module_path in _DAY2_AGENTS:
            ...
            assert (
                entry is not None
            ), f"{name} not found in capability_registry after import"
    ...
    def test_registry_entries_risk_level(self) -> None:
        ...
        for module_path in _DAY2_AGENTS:
            ...
            assert entry is not None
            assert (
                entry.risk_level in _ALLOWED_RISK_LEVELS
...
class TestBugFixFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.bug_fix.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.bug_fix.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.bug_fix.make_bug_fix_handlers", return_value={}
        ):
...
class TestSecurityReviewerFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.security_reviewer.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.security_reviewer.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.security_reviewer.make_security_reviewer_handlers",
            return_value={},
        ):
...
class TestArchitectureReviewerFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            ...
            "app.agents.architecture_reviewer.get_settings",
...
class TestSQLAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.sql_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.sql_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.sql_agent.make_sql_agent_handlers", return_value={}
        ):
...
class TestDockerAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.docker_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.docker_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.docker_agent.make_docker_agent_handlers", return_value={}
        ):
...
class TestCICDAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.cicd_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.cicd_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.cicd_agent.make_cicd_agent_handlers", return_value={}
        ):
...
class TestRefactorAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.refactor_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.refactor_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.refactor_agent.make_refactor_agent_handlers", return_value={}
        ):
            from app.agents.refactor_agent import run_refactor_agent

            run_refactor_agent(
                task_id=7, refactor_instructions="Extract helper function"
            )
            kwargs = mock_run.call_args_list[0][1]
...
class TestReadmeAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.readme_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.readme_agent.get_settings", return_value=_mock_settings()
        ), patch(
...
class TestAPIDocsAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.api_docs_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.api_docs_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.api_docs_agent.make_api_docs_agent_handlers", return_value={}
        ):
...
class TestDependencyAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.dependency_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.dependency_agent.get_settings", return_value=_mock_settings()
        ), patch(
            ...
            run_dependency_agent(
                task_id=10, task_description="Check for outdated packages"
            )
            kwargs = mock_run.call_args_list[0][1]
...
class TestMonitoringAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch(
            "app.agents.monitoring_agent.run_agent_graph", return_value=_FINAL_STATE
        ) as mock_run, patch(
            "app.agents.monitoring_agent.get_settings", return_value=_mock_settings()
        ), patch(
            "app.agents.monitoring_agent.make_monitoring_agent_handlers",
            return_value={},
        ):
