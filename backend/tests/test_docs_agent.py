...
def test_docs_write_md_file_allowed() -> None:
    ...
    with tempfile.TemporaryDirectory() as wt:
        ...
        result = handlers["write_file"](
            {"path": "docs/changelog/2026-07-03-epic.md", "content": "# Change\n"}
        )
...
def test_docs_write_ts_file_denied() -> None:
    ...
    with tempfile.TemporaryDirectory() as wt:
        handlers = _make_handlers(wt)
        result = handlers["write_file"](
            {"path": "apps/web/lib/api.ts", "content": "export const x = 1"}
        )
...
def test_docs_write_py_file_denied() -> None:
    ...
    with tempfile.TemporaryDirectory() as wt:
        handlers = _make_handlers(wt)
        result = handlers["write_file"](
            {"path": "backend/app/main.py", "content": "x = 1"}
        )
        assert "POLICY DENIED" in result
...
def test_docs_write_under_docs_dir_allowed() -> None:
    ...
    with tempfile.TemporaryDirectory() as wt:
        handlers = _make_handlers(wt)
        result = handlers["write_file"](
            {"path": "docs/architecture/overview.md", "content": "# Arch\n"}
        )
        assert "Written" in result
