...
class TestAgentContractFormat:
    ...
    def test_contract_types_are_lists(self, module_path: str) -> None:
        ...
        assert isinstance(c["allowed_tools"], list), f"{module_path}: allowed_tools must be list"
        assert isinstance(c["input_types"], list), f"{module_path}: input_types must be list"
        assert isinstance(c["output_types"], list), f"{module_path}: output_types must be list"
        assert isinstance(c["side_effects"], list), f"{module_path}: side_effects must be list"
        assert isinstance(c["permissions"], list), f"{module_path}: permissions must be list"
        assert isinstance(c["dependencies"], list), f"{module_path}: dependencies must be list"
...
class TestCapabilityRegistration:
    ...
    def test_registry_entries_risk_level(self) -> None:
        ...
        for mp in _DAY3_AGENTS:
            ...
            assert cap.risk_level in _VALID_RISK_LEVELS, (
    ...
    def test_register_idempotent(self) -> None:
        ...
        for mp in _DAY3_AGENTS:
            name = mp.split(".")[-1]
            assert registry.get(name) is not None, f"{name} missing after double register"
...
class TestPerformanceReviewerFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.performance_reviewer.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.performance_reviewer.get_settings", return_value=_mock_settings()), \
             patch("app.agents.performance_reviewer.make_performance_reviewer_handlers", return_value={}):
            from app.agents.performance_reviewer import run_performance_reviewer
            run_performance_reviewer(task_id=1, description="Check for slow SQL queries")
...
class TestStyleReviewerFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.style_reviewer.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.style_reviewer.get_settings", return_value=_mock_settings()), \
             patch("app.agents.style_reviewer.make_style_reviewer_handlers", return_value={}):
...
class TestSprintPlannerFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.sprint_planner.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.sprint_planner.get_settings", return_value=_mock_settings()), \
             patch("app.agents.sprint_planner.make_sprint_planner_handlers", return_value={}):
...
class TestBusinessAnalystFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.business_analyst.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.business_analyst.get_settings", return_value=_mock_settings()), \
             patch("app.agents.business_analyst.make_business_analyst_handlers", return_value={}):
            from app.agents.business_analyst import run_business_analyst
            run_business_analyst(task_id=4, description="Extract user stories for dashboard")
            kwargs = mock_run.call_args_list[0][1]
...
class TestMigrationAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.migration_agent.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.migration_agent.get_settings", return_value=_mock_settings()), \
             patch("app.agents.migration_agent.make_migration_agent_handlers", return_value={}):
...
class TestSchemaAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.schema_agent.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.schema_agent.get_settings", return_value=_mock_settings()), \
             patch("app.agents.schema_agent.make_schema_agent_handlers", return_value={}):
...
class TestAiEngineerFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.ai_engineer.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.ai_engineer.get_settings", return_value=_mock_settings()), \
             patch("app.agents.ai_engineer.make_ai_engineer_handlers", return_value={}):
...
class TestCleanupAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.cleanup_agent.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.cleanup_agent.get_settings", return_value=_mock_settings()), \
             patch("app.agents.cleanup_agent.make_cleanup_agent_handlers", return_value={}):
...
class TestTechDebtAgentFlags:
    def test_fleet_flags(self) -> None:
        with patch("app.agents.tech_debt_agent.run_agent_graph", return_value=_FINAL_STATE) as mock_run, \
             patch("app.agents.tech_debt_agent.get_settings", return_value=_mock_settings()), \
             patch("app.agents.tech_debt_agent.make_tech_debt_agent_handlers", return_value={}):
