...
def test_agent_contract_non_empty_lists(module_name: str) -> None:
    ...
    contract = mod.AGENT_CONTRACT
    assert (
        len(contract["allowed_tools"]) > 0
    ), f"{module_name}: allowed_tools must not be empty"
    assert (
        len(contract["input_types"]) > 0
    ), f"{module_name}: input_types must not be empty"
    assert (
        len(contract["output_types"]) > 0
    ), f"{module_name}: output_types must not be empty"
...
def test_verification_config_enforce_non_empty(module_name: str) -> None:
    ...
    ), f"{module_name} _CFG missing enforce_in_result"
    assert (
        cfg.enforce_in_result
...
@pytest.mark.parametrize(
    "module_name,factory_fn,submit_key",
    [
        (
            "app.agents.code_explainer_agent",
            "make_code_explainer_agent_handlers",
        ...
        (
            "app.agents.api_designer_agent",
            "make_api_designer_agent_handlers",
            "submit_api_designer_agent",
        ),
        (
            "app.agents.compliance_agent",
            "make_compliance_agent_handlers",
            "submit_compliance_agent",
        ),
        (
            "app.agents.cost_estimator_agent",
            "make_cost_estimator_agent_handlers",
